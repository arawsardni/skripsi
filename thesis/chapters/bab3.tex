...
konsisten antarkorpus -- penelitian ini menggunakan skema empat kelas
emosi (marah, senang, netral, sedih) mengikuti kelas yang tersedia pada
dataset target, sehingga perbandingan antarstrategi \emph{fine-tuning}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.8cm} l l p{2.6cm} p{1.8cm}}
    ...
    E-SERAVD & Target & Indonesia & [\emph{lihat Santoso et al., 2026}] & 4 \\
...
resmi masing-masing korpus. Untuk dataset target, skala yang tersedia
dari tim yang sama (kemungkinan besar E-SERAVD atau versi awalnya)
adalah 5.049 sampel dengan distribusi 1.202 marah, 1.228 senang, 2.075
netral, dan 899 sedih -- angka ini belum tentu utuh berlaku untuk
E-SERAVD maupun IndoWaveSentiment secara spesifik, sehingga perlu
diverifikasi ulang terhadap dokumentasi resmi masing-masing dataset
sebagaimana dilaporkan oleh Santoso, Budianto and Dutono (2026) dan
Nugroho et al. (2025) sebelum dataset benar-benar diperoleh.
...
diterapkan pada level adaptasi target ini untuk keempat kondisi.
...
pada tiap kondisi. Kedua metrik ini dibandingkan secara deskriptif
...
\begin{table}[H]
  ...
  \begin{tabular}{l l l}
    \toprule
    \textbf{Komponen} & \textbf{Full Fine-Tuning} & \textbf{LoRA} \\
    \midrule
    Bobot + master + gradien + optimizer Adam & $\sim$5 GB & $\sim$0,6--0,7 GB \\
    Sisa untuk aktivasi (dari 6 GB)           & $\sim$1 GB & $\sim$5,3 GB \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent
Sisa VRAM untuk aktivasi pada skema \emph{full fine-tuning} jauh lebih
ketat dibandingkan LoRA, sehingga \emph{full fine-tuning} diperkirakan
memerlukan \emph{batch size} kecil (sekitar 1--2) dengan \emph{gradient
checkpointing} sebagai teknik wajib, kemungkinan disertai \emph{gradient
accumulation} untuk mencapai \emph{batch size} efektif yang lebih wajar.
Skema LoRA memiliki ruang gerak yang jauh lebih longgar, dengan
\emph{batch size} 8--16 yang tetap realistis tanpa teknik tambahan.
Keterbatasan ini sekaligus menjadi ilustrasi konkret argumen efisiensi
LoRA yang diuji dalam penelitian ini (Subbab \ref{sec:2.7}): pada
perangkat keras yang benar-benar terbatas, LoRA bukan hanya lebih hemat
secara teoretis, tetapi menjadi opsi yang jauh lebih praktis dijalankan.
